...
const STATUS_FILTERS = ["All", "At Risk", "In Transit", "Delivered", "Exception"] as const;
...
export function LoadsTableClient({ loads }: { loads: LoadRow[] }) {
  ...
  const activeFilterCount = (riskFilter !== "all" ? 1 : 0) + (customerFilter !== "all" ? 1 : 0);
  ...
  const filtered = useMemo(() => {
    ...
    return loads.filter((l) => {
      ...
      const passStatus =
        statusFilter === "All" ||
        (statusFilter === "At Risk"    && (risk === "high" || risk === "critical")) ||
        ...
        <div style={{ position: "relative" }} ref={panelRef}>
          <button
            ...
            style={{
              ...
              fontSize: 12, fontWeight: activeFilterCount > 0 ? 600 : 400, cursor: "pointer",
            }}
          >
            ...
            {activeFilterCount > 0 && (
              <span style={{
                background: "#2563EB", color: "#FFFFFF", borderRadius: "50%",
            ...
            <div style={{
              ...
              <div style={{ borderTop: "1px solid #F2F2F2", paddingTop: 14 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                  {RISK_OPTIONS.map(({ label, value }) => {
                    ...
                    return (
                      <button key={value} type="button" onClick={() => setRiskFilter(value)} style={{
              ...
              {activeFilterCount > 0 && (
                <div style={{ borderTop: "1px solid #F2F2F2", paddingTop: 10, marginTop: 14 }}>
                  <button type="button" onClick={() => { setRiskFilter("all"); setCustomerFilter("all"); setFilterOpen(false); }} style={{
